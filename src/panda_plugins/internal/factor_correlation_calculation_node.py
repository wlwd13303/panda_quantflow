...
class FactorCorrelationOutputModel(BaseModel):
    ...
    因子相关性分析输出模型
    """
    factor_correlation: object = Field(title="因子组", description="合并后的DataFrame")
    
    model_config = {"arbitrary_types_allowed": True}

    @field_validator('factor_correlation')
    def validate_factor_correlation(cls, v):
        if not isinstance(v, pd.DataFrame):
            raise ValueError('factor_correlation must be a pandas DataFrame')
        return v
...
class FactorCorrelationCalculationControl(BaseWorkNode):
    ...
    def run(self, input: BaseModel) -> BaseModel:
        ...
        Returns:
            包含因子两俩相关性的DataFrame矩阵 (列: 因子名1、因子名2、因子名3, 行: 因子名1、因子名2、因子名3)
        """
        try:
            # 获取所有非空的因子 DataFrame
        ...
        try:
            factor_correlation = merged_df.corr(numeric_only=True)

            return FactorCorrelationOutputModel(factor_correlation=factor_correlation)
        except:
            logger.error(f"因子相关性分析失败: {str(e)}")
            raise ValueError(f"因子相关性分析失败: {str(e)}")
...
if __name__ == "__main__":
    ...
    print("\n合并后的结果:")
    print("形状:", result.factor_correlation.shape)
    print("列名:", list(result.factor_correlation.columns))
    print("\n前几行数据:")
    print(result.factor_correlation.head(10))