...
from panda_backtest.FutureTest import start
...
class FutureBacktestInputModel(BaseModel):
    ...
    # future_account_id:str=Field(default="8888",title="期货户")
...
class FutureBacktestControl(BaseWorkNode):
    ...
    def run(self, input: BaseModel) -> BaseModel:
        backtest_id=start(code=input.code,start_future_capital=input.start_future_capital,future_account_id="8888",start_date=input.start_date,end_date=input.end_date)
