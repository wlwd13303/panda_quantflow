...
class DateUtil(object):
    ...
    @classmethod
    def get_pre_date(cls, trade_date:int):
        trade_cal_col = cls.__quotation_db.trade_calendar
        pre_trade_date_cur = trade_cal_col.find( {'nature_date': {'$lt': int(trade_date)}, 'is_trade': 1, "exchange": "SH"}).sort([('nature_date', -1)]).limit(1)
    ...
    @classmethod
    def get_next_trade_date(cls, trade_date:int, operate='$gt'):
        collection = cls.__quotation_db.trade_calendar
        next_date_cur = collection.find({
            'nature_date': {operate: int(trade_date)}, 'is_trade': 1, "exchange": "SH"}).sort('nature_date',
